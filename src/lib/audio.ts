...
class SoundEngine {
  ...
  // Rekindling cosmic revival chord on streak restore
  public playRestoreStreak() {
    if (this.isMuted) return;
    this.initCtx();
    if (!this.ctx) return;

    const now = this.ctx.currentTime;
    const notes = [329.63, 440.0, 554.37, 659.25, 880.0]; // E4, A4, C#5, E5, A5
    notes.forEach((freq, idx) => {
      ...
      osc.type = "triangle";
      osc.frequency.setValueAtTime(freq * 0.7, now + idx * 0.08);
      osc.frequency.exponentialRampToValueAtTime(freq, now + idx * 0.08 + 0.2);
